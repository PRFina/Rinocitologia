...
def extraction_cells(image, k, patient):
    # if the directory doesn't exist then create a new one
    in_path = os.getcwd() + "/"
    # directory = in_path + "/" + "cellule/"          # path dove finiscono le cellule
    directory = patient.getPathCellule()
    print(directory)
    ...
    for region in regionprops(filtered_labels):
        ...
        if i != 0:
            print(io.imsave(patient.getPathCellule() + "/Img_" + str(k) + "_cell" + str(i) + ".png", cell))
            io.imsave(patient.getPathCellule() + "/Img_" + str(k) + "_cell" + str(i) + ".png", cell)
...
if __name__ == "__main__":
    ...
    for infile in glob.glob(os.path.join(path, '*.png')):
        ...
        try:
            extraction_cells(img_or, c, patient)
        except ValueError:
            continue
        c = c + 1
